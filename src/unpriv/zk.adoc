...
This chapter describes the _scalar_ cryptography
...
This chapter also describes the architectural interface to an
...
We expect the sections on <<ext:zkt, constant time execution>>
and the <<crypto_scalar_es, entropy source>>
...
Note that this chapter contains only a subset of the formal model: refer to
...
[[ext:zknd]]
===== ext:zknd[] NIST Suite: AES Decryption
...
[[ext:zkne]]
===== ext:zkne[] NIST Suite: AES Encryption
...
[[ext:zknh]]
===== ext:zknh[] NIST Suite: Hash Function Instructions
...
[[ext:zksed]]
===== ext:zksed[] ShangMi Suite: SM4 Block Cipher Instructions
...
[[ext:zksh]]
===== ext:zksh[] ShangMi Suite: SM3 Hash Function Instructions
...
[[ext:zkr]]
===== ext:zkr[] Entropy Source Extension

[[norm:zkr_seed_addr]]
The entropy source extension defines the csr:seed[] CSR at address `0x015`.
This CSR provides up to 16 physical csr::[entropy] bits that can be used to
...
| extlink:zbkb[]  | Bitmanipulation instructions for cryptography.
| extlink:zbkc[]  | Carry-less multiply instructions.
| extlink:zbkx[]  | Cross-bar Permutation instructions.
| extlink:zkne[]  | AES encryption instructions.
| extlink:zknd[]  | AES decryption instructions.
| extlink:zknh[]  | SHA2 hash function instructions.
|===

[[norm:zkn_impl_all]]
A core which implements ext:zkn[] must implement all of the above extensions.

[[ext:zks]]
===== ext:zks[] ShangMi Algorithm Suite
...
| extlink:zbkc[]  | Carry-less multiply instructions.
| extlink:zbkx[]  | Cross-bar Permutation instructions.
| extlink:zksed[] | SM4 block cipher instructions.
| extlink:zksh[]  | SM3 hash function instructions.
|===

[[norm:zks_impl_all]]
A core which implements ext:zks[] must implement all of the above extensions.

[[ext:zk]]
===== ext:zk[] Standard scalar cryptography extension
...
| extlink:zkn[]  | NIST Algorithm suite extension.
| extlink:zkr[]  | Entropy Source extension.
| extlink:zkt[] | Data independent execution latency extension.
|===

[[norm:zk_impl_all]]
A core which implements ext:zk[] must implement all of the above extensions.
...
Description::
This instruction sources a single byte from _rs2_ according to _bs_.
...
MixColumn, before XOR'ing the result with _rs1_.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
before XOR'ing the result with _rs1_.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
Note that _rnum_ must be in the range `0x0`{endash}``0xA``.
The values `0xB`{endash}``0xF`` are reserved.
...
Description::
insn:clmul[] produces the lower half of the 2{times}XLEN carry-less product.
...
Description::
insn:clmulh[] produces the upper half of the 2{times}XLEN carry-less product.
...
result is sign-extended to XLEN bits.
...
Operation::
+
sail::execute[clause="SHA256SIG0(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA256SIG1(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA256SUM0(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA256SUM1(_, _)",part=body,unindent]
...
in conjunction with the insn:sha512sig0l[] instruction.
...
Operation::
+
sail::execute[clause="SHA512SIG0H(_, _, _)",part=body,unindent]
...
This instruction is implemented on RV32 only.
Used to compute the Sigma0 transform of the SHA2-512 hash function
in conjunction with the insn:sha512sig0h[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Operation::
+
sail::execute[clause="SHA512SIG0L(_, _, _)",part=body,unindent]
...
in conjunction with the insn:sha512sig1l[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Operation::
+
sail::execute[clause="SHA512SIG1H(_, _, _)",part=body,unindent]
...
This instruction is implemented on RV32 only.
Used to compute the Sigma1 transform of the SHA2-512 hash function
in conjunction with the insn:sha512sig1h[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Operation::
+
sail::execute[clause="SHA512SIG1L(_, _, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SUM0R(_, _, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SUM1R(_, _, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SIG0(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SIG1(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SUM0(_, _)",part=body,unindent]
...
Operation::
+
sail::execute[clause="SHA512SUM1(_, _)",part=body,unindent]
...
SM4 round function.
A byte is extracted from _rs2_ based on _bs_, to which the SBox and
linear layer transforms are applied, before the result is XOR'd with
_rs1_ and written back to _rd_.
...
It is the inverse of the insn:zip[] instruction.
...
Operation::
+
sail::execute[clause="XPERM8(_, _, _)",part=body,uindent]
...
It is the inverse of the insn:unzip[] instruction.
This instruction is available only on RV32.
...
The csr:seed[] CSR provides an interface to a NIST SP 800-90B cite:[TuBaKe:18]
...
Bit Generator (RBG), but can be used to build standard (and non-standard)
...
to create random bits securely cite:[BaKeMc:25].
...
[#norm:seed_csr_unpriv]#csr:seed[] is an unprivileged CSR located at address `0x015`.#
The 32-bit contents of csr:seed[] are as follows:

[%autowidth.stretch,cols="^,^,<",options="header",]
|=======================================================================
|Bits |Name |Description

|`31:30` |csr::[opst] |Status: `BIST` (00), `WAIT` (01), `ES16` (10), `DEAD`
(11).
...
|`15: 0` |csr::[entropy] |16 bits of randomness, only when `OPST=ES16`.
|=======================================================================

[#norm:seed_ro_illegal]#Attempts to access the csr:seed[] CSR using a read-only CSR-access instruction
(insn:csrrs[]/insn:csrrc[] with _rs1_=`x0` or insn:csrrsi[]/insn:csrrci[] with _uimm_=0) raise an
illegal-instruction exception; any other CSR-access instruction may be used
to access csr:seed[].#
[#norm:seed_write_ignore]#The write value (in _rs1_ or _uimm_) must be ignored by implementations.#
The purpose of the write is to signal polling and flushing.

Software normally uses the instruction insn:csrrw[rd,seed,x0] to read the csr:seed[]
CSR.
...
[#norm:seed_exec_mode_control]#The csr:seed[] CSR is also access controlled by execution mode, and attempted
...
The status bits csr:seed[][31:30] = csr::[opst] may be `ES16` (10),
indicating successful polling, or one of three entropy polling failure
statuses `BIST` (00), `WAIT` (01), or `DEAD` (11), discussed below.

[#norm:seed_entropy_unique]#Each returned csr:seed[][15:0] = csr::[entropy] value represents unique randomness
when csr::[opst]=`ES16` (`seed[31:30]` = `10`), even if its numerical value is
the same as that of a previously polled csr::[entropy] value.# The implementation
requirements of csr::[entropy] bits are defined in <<crypto_scalar_es_req>>.
[#norm:seed_entropy_zero_non_es16]#When csr::[opst] is not `ES16`, csr::[entropy] must be set to 0.#
An implementation may safely set reserved and custom bits to zeros.

[#norm:seed_wipe_on_read]#For security reasons, the interface guarantees that secret
csr::[entropy] words are not made available multiple times. Hence polling (reading) must
also have the side effect of clearing (wipe-on-read) the csr::[entropy] contents and
changing the state to `WAIT` (unless there is csr::[entropy] immediately
available for `ES16`). Other states (`BIST`, `WAIT`, and `DEAD`)
may be unaffected by polling.#

The Status Bits returned in csr:seed[][31:30]=csr::[opst]:

* `00` - `BIST`
indicates that Built-In Self-Test "`on-demand`" (BIST) testing is being
performed. If csr::[opst] returns temporarily to `BIST` from any other
...
* `10` - `ES16`
indicates success; the low bits csr:seed[][15:0] will have 16 bits of
randomness (csr::[entropy]), which is guaranteed to meet certain minimum entropy
...
(no data) and `ES16`, which means that 16 bits of randomness (csr::[entropy])
...
The output csr::[entropy] (csr:seed[][15:0] in ES16 state) is not necessarily
...
obtained from each 256-bit (16{times}16-bit) successful, but possibly
non-consecutive csr::[entropy] (ES16) output sequence using a vetted conditioning
...
least 128+64 = 192 bits, as discussed in SP 800-90C cite:[BaKeMc:25];
...
at least 512 csr::[entropy] bits to initialize a DRBG that has 256-bit security.
...
A virtual source traps access to the csr:seed[] CSR, emulates it, or
...
[#norm:seed_min_security_256]#Any implementation of the csr:seed[] CSR that limits the security
strength shall not reduce it to less than 256 bits.# [#norm:seed_disable_if_weak]#If the security
...
attempted access to csr:seed[] and feed a less privileged client
...
(<<crypto_scalar_es_req_ptg2>>) _physical entropy source_. Emulated csr:seed[]
...
Implementations may implement csr:mseccfg[] such that csr::[sseed] and csr::[useed] is a read-only
constant value `0`. Software may discover if access to the csr:seed[] CSR can be
enabled in U and S mode by writing a `1` to csr::[sseed] and csr::[useed] and reading back
the result.
...
subset of the instructions from the Bitmanip extensions ext:zba[], ext:zbb[], and ext:zbc[] directly.
Specifically, this would mean that
a core implementing
_either_
the scalar cryptographic extensions,
_or_
the ext:zba[], ext:zbb[], and ext:zbc[],
_or_
both,
...
See extlink:zbkb[] for details of these instructions.
...
rotation instructions, so insn:zip[] and insn:unzip[] are not required.
====
...
See extlink:zbkc[] for details of these instructions.
...
The insn:andn[] instruction is also useful for constant time word-select
in systems without the ternary Bitmanip insn:cmov[] instruction.
...
[NOTE,caption="SH"]
====
The insn:pack*[] instructions are
...
around as (potentially unaligned) byte strings, but can operate on words
...
See extlink:zbkx[] for a complete description of these instructions.
...
time, using the insn:xperm8[] instruction
...
* *ISA Architectural Tests.* Verify, to the extent possible, that RISC-V ISA
 ...
 (<<crypto_scalar_es_access>>), and that csr:seed[] ES16 csr::[entropy] words
...
The csr:seed[] CSR provides an Entropy Source (ES) interface, not a stateful
...
embedded firmware, or a cryptographic  library) may use csr:seed[] to
...
thousands of cycles under some circumstances. A insn:wfi[] or insn:pause[]
mechanism (at least potentially) allows energy-saving sleep on MCUs
and context switching on higher-end CPUs.

The reason for the particular csr::[opst] = csr:seed[][31:0] two-bit mechanism is to
...
SP 800-90C cite:[BaKeMc:25] states that each conditioned block of n bits
...
(cite:[BaKeMc:25], Sections 4.1. and 4.3.2).
Only then a hashing of 16{times}16 = 256 bits from the entropy source
...
contained in SP 800-90C  cite:[BaKeMc:25], Appendix A.
...
full entropy sources cite:[BaKeMc:25]. The concatenation of output words
corresponds to the `Get_entropy_bitstring` function.
...
The direct csr:mseccfg[sseed] and mseccfg[useed] options allow one to draw a security boundary
...
one way of doing it is to model csr:seed[] as a rejection
...
csr::[entropy] bits themselves. If it does, additional countermeasures are
necessary.
...
processed random words shall be made available via csr::[entropy] (ES16 status
of csr:seed[]).
...
Source (ES) and provided by the csr:seed[] CSR.
...
complex and energy-hungry to implement solely to serve the csr:seed[] CSR.
...
implemented as a custom, M-mode only CSR, denoted here as csr:mnoise[].

This non-normative interface is not intended to be used as a source of
randomness or for other production use.
We define the semantics for single bit for this interface, csr:mnoise[][31],
which is named csr::[noise_test], which will affect the behavior of csr:seed[]
if implemented.

When csr::[noise_test]=1 in csr:mnoise[], the csr:seed[] CSR must not return
anything via `ES16`; it should be in `BIST` state unless the source
is `DEAD`. When csr::[noise_test] is again disabled, the entropy source
...
as is the address of the custom csr:mnoise[] CSR. Other contents and behavior of the
CSR only can be interpreted in the context of csr:mvendorid[], csr:marchid[], and
csr:mimpid[] CSR identifiers.

When not implemented (e.g., in virtual machines), csr:mnoise[] can permanently
read zero (`0x00000000`) and ignore writes.
When available, but csr::[noise_test]=0, csr:mnoise[] can return a
...
In csr::[noise_test] mode, the WAIT and ES16 states are unreachable,
and no entropy is output. Implementation of test interfaces that directly
affect ES16 entropy output from the csr:seed[] CSR interface is discouraged.
...
While this chapter contains the specifications for the RISC-V cryptography
extensions, numerous supplementary materials and example codes have
also been developed.
All of the supplementary materials related to the RISC-V Cryptography
