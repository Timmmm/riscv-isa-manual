...
The instructions it adds are collectively referred to as insn:push[]/insn:pop[]:
...
The term insn:push[] refers to insn:cm.push[].

The term insn:pop[] refers to insn:cm.pop[].

The term insn:popret[] refers to insn:cm.popret[] and insn:cm.popretz[].

Common details for these instructions are in this section.

==== insn:push[]/insn:pop[] functional overview

insn:push[], insn:pop[], insn:popret[] are used to reduce the size of function prologues and epilogues.

. The insn:push[] instruction
...
This example gives an illustration of the use of insn:push[] and insn:popret[].
...
with insn:push[]/insn:popret[] this reduces to
...
and to 4-bytes with insn:push[] and insn:popret[].
As well as reducing the code-size insn:push[] and insn:popret[] eliminate the branches from
...
[NOTE]
====
insn:pop[] is typically used in tail-calling sequences where _ret_ is not used to return to _ra_ after destroying the stack frame.
====
...
range available in the encoding; if the range is insufficient then a separate insn:c.addi16sp[] can be used to increase the range.
...
==== insn:push[]/insn:pop[] Fault handling
...
The entire insn:push[]/insn:pop[] sequence is re-executed after returning from the trap handler, and multiple traps are possible during the sequence.
...
===== Software view of the insn:push[] sequence

From a software perspective the insn:push[] sequence appears as:
...
[#norm:Zcmp_push_sp_commit]#The stack pointer adjustment must only be committed only when it is certain that the entire insn:push[] instruction will commit.#
...
or handles errors responses after completing the insn:push[] instruction.
...
===== Software view of the insn:pop[]/insn:popret[] sequence

From a software perspective the insn:pop[]/insn:popret[] sequence appears as:
...
* A stack pointer adjustment
* An optional insn:li[a0,0]
* An optional insn:ret[]
...
[#norm:Zcmp_pop_sp_commit]#The optional insn:li[a0,0], stack pointer adjustment and optional insn:ret[] must only be committed only when it is certain that the entire insn:pop[]/insn:popret[] instruction will commit.#

For insn:popret[] once the stack pointer adjustment has been committed the insn:ret[] must execute.
...
An implementation may have a requirement to issue a insn:push[]/insn:pop[] instruction to non-idempotent memory.

If the core implementation does not support insn:push[]/insn:pop[] to non-idempotent memories, the core may use an idempotency PMA to detect it and take a
load (insn:pop[]/insn:popret[]) or store (insn:push[]) access-fault exception in order to avoid unpredictable results.
...
==== Example RV32I insn:push[]/insn:pop[] sequences

The examples are included show the load/store series expansion and the stack adjustment.
Examples of insn:cm.popret[] and insn:cm.popretz[] are not included, as the difference in the expanded sequence from insn:cm.pop[] is trivial in all cases.

===== insn:cm.push[\{ra, s0-s2},-64]
...
insn:cm.push[\{reg_list},-stack_adj]
...
[NOTE]
====
_rlist_ values 0 to 3 are reserved for a future EABI variant called insn:cm.push.e[]
====
...
insn:cm.pop[\{reg_list},stack_adj]
...
[NOTE]
====
_rlist_ values 0 to 3 are reserved for a future EABI variant called insn:cm.pop.e[]
====
...
insn:cm.popretz[\{reg_list},stack_adj]
...
[NOTE]
====
_rlist_ values 0 to 3 are reserved for a future EABI variant called insn:cm.popretz.e[]
====
...
[NOTE]
====
The insn:li[a0,0] *could* be executed more than once, but is included in the atomic section for convenience.
====
...
insn:cm.popret[\{reg_list},stack_adj]
...
[NOTE]
====
The _s_ register mapping is taken from the UABI, and may not match the currently unratified EABI. insn:cm.mvsa01.e[] may be included in the future.
====
...
[NOTE]
====
The _s_ register mapping is taken from the UABI, and may not match the currently unratified EABI. insn:cm.mva01s.e[] may be included in the future.
====
